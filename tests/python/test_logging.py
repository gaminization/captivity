...
def test_json_formatter_standard_fields():
    ...
    record = logging.LogRecord(
        "test_logger", logging.INFO, "test.py", 10, "Hello %s", ("World",), None
    )
    ...
    assert (
        data["timestamp"]
        == datetime.fromtimestamp(1600000000.0, tz=timezone.utc).isoformat()
    )
...
def test_json_formatter_extra_fields():
    formatter = JSONFormatter()
    record = logging.LogRecord(
        "test_logger", logging.WARNING, "test.py", 10, "User login", (), None
    )
...
def test_json_formatter_with_exception():
    ...
    record = logging.LogRecord(
        "test_logger", logging.ERROR, "test.py", 10, "Math error", (), exc_info
    )
