...
class TestFormalProperties(unittest.TestCase):
    ...
    def test_probe_dominates_events(
        self,
        probe_statuses: List[ConnectivityStatus],
        ...
        with unittest.mock.patch(
            "captivity.daemon.runner.probe_connectivity_detailed"
        ) as mock_probe:
            with unittest.mock.patch("captivity.daemon.runner.do_login") as mock_login:
                ...
                for event in network_events:
                    ...
                    )  # Arbitrary for the event handler
                    ...
                    probe_status = probe_statuses[
                        len(probe_statuses) % len(probe_statuses)
                    ]
                    ...
                    if probe_status == ConnectivityStatus.CONNECTED:
                        self.assertEqual(
                            runner.state_machine.state, ConnectionState.CONNECTED
                        )
                    elif probe_status == ConnectivityStatus.NETWORK_UNAVAILABLE:
                        self.assertEqual(
                            runner.state_machine.state, ConnectionState.ERROR
                        )
                        ...
                        self.assertIn(
                            ...
                            {ConnectionState.ERROR, ConnectionState.WAIT_USER},
                        )
