...
def mock_data_dir(monkeypatch):
    ...
    monkeypatch.setattr(
        "captivity.telemetry.stats.STATS_FILE", temp_path / "stats.json"
    )
    monkeypatch.setattr(
        "captivity.core.cache.CACHE_FILE", temp_path / "portal_cache.json"
    )
    monkeypatch.setattr(
        "captivity.core.profiles.PROFILES_FILE", temp_path / "profiles.json"
    )
